/*
  Lonely Binary ESP32 with 1.8" Full Color LCD
  Demo Code

  Pin Configuration:
  Copy "User_Setup.h" into "TFT_eSPI" library folder
...
uint32_t testHaD()
{
  ...
  tft.setCursor(0, 145);
  tft.println(F("LONELY BINARY"));
  tft.println(F("ESP32 & 1.8-Inch LCD"));
...
void loop(void)
{
  ...
  tft.println(F("ESP32 1.8\" TFT TEST"));
  tft.println(F(""));
  tft.setTextSize(1);
